  ...
  stages {
    ...
    stage('Build') {
      steps {
        ...
        dir('tmp') {
          timestamps {
            sh '../scripts/firedrake-install --disable-ssh --minimal-petsc --slepc --documentation-dependencies --install thetis --install gusto --install icepack --install irksome --no-package-manager --package-branch loopy cvec-restructure-checks --package-branch PyOP2 vectorisation-restructure-checks|| (cat firedrake-install.log && /bin/false)'
          }
        }
      }
    }

  ...
  stages {
    ...
    stage('Build') {
      steps {
        ...
        dir('tmp') {
          timestamps {
            sh '../scripts/firedrake-install --disable-ssh --minimal-petsc --slepc --documentation-dependencies --install thetis --install gusto --install icepack --install irksome --no-package-manager --package-branch loopy allow-callablestables-gen-with-functionsnotinhistory --package-branch PyOP2 inline-callables-from-loopy-programs || (cat firedrake-install.log && /bin/false)'
          }
        }
      }
    }
